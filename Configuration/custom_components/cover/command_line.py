...
COVER_SCHEMA = vol.Schema({
    ...
    vol.Optional(CONF_COMMAND_POSITION): cv.string,
    ...
    for device_name, device_config in devices.items():
        ...
        covers.append(
            CommandCover(
                ...
                device_config.get(CONF_COMMAND_POSITION),
...
class CommandCover(CoverDevice):
    ...
    def __init__(self, hass, name, command_open, command_close, command_stop,
                 command_position, command_state, value_template):
        ...
        self._command_position = command_position
    ...
    def set_cover_position(self, **kwargs):
        ...
        if ATTR_POSITION in kwargs:
            ...
            if 0 <= int(position) <= 100:
                command = self._command_position.replace('{position}', str(position))
                self._move_cover(command)
            else:
                _LOGGER.warning(
