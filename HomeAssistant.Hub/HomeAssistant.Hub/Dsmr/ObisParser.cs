    ...
    public class ObisParser
    {
        ...
        public async Task ParseFromStream(Stream stream, TelegramParsedEventHandler onParsedEvent)
        {
            ...
            while ((count = await stream.ReadAsync(buffer, 0, buffer.Length)) > 0)
            {
                if (count != 0)
                {
                    using (StringReader reader = new StringReader(TelegramEncoding.GetString(buffer)))
                    {
                        await ParseFromStringReader(reader, onParsedEvent);
                    }
                }
            }
            logger.Warn("Stream disappeared somehow");
        }
        ...
        public async Task ParseFromStringReader(StringReader reader, TelegramParsedEventHandler onParsedEvent)
        {
            ...
            while ((line = await reader.ReadLineAsync()) != null)
            {
                if (telegram == null)
                {
                    if (line.StartsWith(telegramStart.ToString(), StringComparison.InvariantCultureIgnoreCase))
                    {
                ...
                {
                    if (line.StartsWith(telegramEnd.ToString(), StringComparison.InvariantCultureIgnoreCase))
                    {
        ...
        private static IEnumerable<string> GetPropertiesWithKey(string key)
        {
            ...
            PropertyInfo[] telegramProperties = typeof(Telegram).GetProperties();
            foreach (PropertyInfo property in telegramProperties)
            {
        ...
        private static void SetTelegramHeader(ref Telegram telegram, string value)
        {
            ...
            var propertyInfo = typeof(Telegram).GetProperty(nameof(Telegram.MessageHeader));
        ...
        private static void SetTelegramCRC(ref Telegram telegram, string value)
        {
            if (telegram.MessageVersion == ObisVersion.V42 || telegram.MessageVersion == ObisVersion.V50)
            {
                ...
                var propertyInfo = typeof(Telegram).GetProperty(nameof(Telegram.CRC));
        ...
        {
            IEnumerable<PropertyInfo> telegramProperties = typeof(Telegram).GetProperties().Where(p => properties.Contains(p.Name));
